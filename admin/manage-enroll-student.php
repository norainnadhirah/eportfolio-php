...
<html lang="en">
      <head>
        ...
        <title>PutraMD E-Portfolio | Manage Enroll Student</title>
        ...
        <?php include_once('include/sidebar.php');?>
                ...
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Manage Enroll Student</h1>
...
$_SESSION['id_program']=$pidprogram;
...
$query = $dbh -> prepare($sql);
$query->bindParam(':idprogram',$_SESSION['id_program'],PDO::PARAM_STR);
$query->execute();
...
if($result){
            ...
            <div class="card-header">  <i class="fa fa-users me-1"></i> Student Taking <?php 

            echo htmlentities($result->ProgramName); ?>(<?php echo $result->ProgramCode; echo $pidprogram; }?>)</div>
            <div class="card-body">
...
WHERE tblstudent_program.IDProgram=:idprogram AND tblstudent_mentor.Status='Approved' AND user.role='Student'";
                                        $query = $dbh -> prepare($sql);
...
<div class=" card-footer d-flex form-floating">
                             <button class="btn btn-default bg-info sbmt-btn" type="submit" name="submit_std" onclick="return confirm('Do you want to remove these students?');">Remove Students</button>
...
if (isset($_POST['submit_std'])) {
  if (isset($_POST['idstudent'])) {
    try {
      ...
      $program = $_SESSION['id_program'];

      $sql = "DELETE FROM tblstudent_program WHERE IDStudent = :IDStudent AND IDProgram = :IDProgram";
      $query = $dbh->prepare($sql);
      $query->bindParam(':IDProgram', $program, PDO::PARAM_STR);
      foreach ($selected_values as $value) {
          $query->bindParam(':IDStudent', $value, PDO::PARAM_STR);
          $query->execute();
      }
      $dbh->commit();
      echo "<script>alert('Removal is Success');</script>";


      exit();
    } catch (PDOException $e) {
      $dbh->rollBack();
      echo "<script>alert('Removal is NOT Success');</script>";
    }
  }
}
